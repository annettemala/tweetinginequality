...
def random_sample(df):
    ...
    print('Drew subsample')
    return df

# Get subset
def subset(df):
    df = df[df['topics'] == True]
...
def preprocess(df, lemmatize, stem, target_column = 'manual frame'):
    ...
    df = clean_dataset.drop_duplicates (df, 'text')
    ...
    # Remove stopwords
    df = prepare_tweets.remove_stopwords_text(df, 'processed text', keepthan = True)
    print('     Removed stopwords.')
        
    # Remove punctuation
    df = prepare_tweets.remove_punctuation_text(df, 'processed text')
    print('     Removed punctuation.')
        
    if lemmatize == True:
        # Lemmatize tweets
        df = prepare_tweets.lemmatize_tweets_text(df, 'processed text')
        print('     Lemmatized tweets.')
            
    if stem == True:
        # Stem tweets
        df = prepare_tweets.stem_tweets_text(df, 'processed text')
        print('     Stemmed tweets.')
    
    # Tokenize tweets
    df = prepare_tweets.tokenize_tweets_text(df, text_column = 'processed text', target_column = 'tokens')
...
# Get only sentences with than
def get_sents_with_than(sentences):
    for sentence in sentences:
        for token in sentence:
            if token.text.lower() == "than":
                return " ".join([token.text for token in sentence])

def get_comparatives(texts):
    nlp = spacy.load('en_core_web_sm')
        
    sentences = texts.apply(lambda x: nlp(x).sents)
    comparatives = sentences.apply(get_sents_with_than)
    comparatives.isna().sum()       # no NAs
    print('Found comparative sentences.')
    return comparatives
...
def lsa_transform (X_tfidf_values):
    svd = TruncatedSVD(100)
    lsa = make_pipeline(svd, Normalizer(copy=False))
    
    # Run SVD on the data
    X_lsa_values = lsa.fit_transform(X_tfidf_values)
...
def cross_val5(df, target, optimized, lsa, n = 2):
    # Output file
    ...
    for i in range(len(names_vectorizers)):
        ...
        if vec == 'tfidf':
            if lsa == True:
                X_values = lsa_transform(X_values)
        
        # Do not use naive bayes after LSA
        if ((vec == 'tfidf') and (lsa == True)):
            names_classifiers.pop()
...
################# Predictions #################
def predict_values(classifier, X_values, y):
    y_pred = cross_val_predict(classifier, X_values, y, cv=5)
...
def compute_accuracy(y_pred, X_test, y_test, classifier, vectorizer, pipe, f):
    # Confusion matrix
...
################# Learning curve #################
def plot_learning_curve(X_train, y_train, pipe):   
...
################# Grid search #################  
def grid_search_params(X_train, y_train, X_test, y_test, pipe):   
    # Creating pipeling
    pipe = Pipeline(
                    [
    ...
    parameters = {
                  ...
                  #"clf__penalty": ("l2", "elasticnet"), #regularization parameters
                  #'clf__max_iter': (10, 20, 50, 1000),
                  'svc__C' : [1,10,100,100],
                  'svc__max_iter' :[10, 100, 1000]
                  }
    
    grid_search = GridSearchCV(pipe, parameters, n_jobs=-1, verbose=1, scoring = "average_precision")
    # or: scoring = 'precision_macro'
    ...
    # Classification report of predicted values 
...
################# Run classification #################
def classification(df, target, vec, optimized, classify, pipe_name, match, n = 2):  
    ...
    f = open('accuracy_output_' + pipe_name + '.txt', 'w')
    ...
    compute_accuracy(y_pred, X_test, y_test, classifier, vectorizer, pipe, f)
        
    # Learning curve
    plot_learning_curve(X_train, y_train, pipe)
    ...
    # Grid search
    #grid_search_params(X_train, y_train, X_test, y_test, pipe)
       
    return df

################# Topic modeling #################
def display_topics(model, feature_names, no_top_words, f):
...
def remove_keywords(df, file_name):
    if file_name == 'race':
        kw_adv = 'white'
        kw_dis = 'black'
        other = ('than', 'asian', 'american', 'latino', 'latina', 'latinx', 'brown', 'african', 'hispanic',
                 'people', 'ppl', 'person', 'guy', 'dude', 'women', 'men', 'woman', 'man')
    elif file_name == 'money':
        kw_adv = 'rich'
        kw_dis = 'poor'
        other = ('than', 'people', 'ppl', 'person', 'guy', 'dude', 'women', 'men', 'woman', 'man')
    elif file_name == 'gender':
        kw_adv = 'men'
        kw_dis = 'women'
        other = ('than', 'people', 'ppl', 'person', 'guy', 'dude', 'women', 'men', 'woman', 'man')
...
def word_cloud_sklearn(index, lda_model, vocab, file_group):
    imp_words_topic=""
    ...
    plt.axis("off")
    plt.tight_layout()
    plt.savefig('wordcloud_' + file_group + str(index) + '.png')
    plt.show()
    print('Created wordcolud ', str(index))
    
def grid_search_lda(tf):
    # Define Search Param
    search_params = {'n_components': [4, 6, 8], 
                     'learning_decay': [.5, .7, .9],
                     'doc_topic_prior':[0.01, 0.1],
    ...
    # Best parameters for race are: {'doc_topic_prior': 0.1 [0.1,1], 
                                     #'learning_decay': 0.9 [0.5,0.9], 
                                     #'n_components': 4 [4,20], 
                                     #'topic_word_prior': 1 [0.1,1]}

def analyze_topics_lda(df, file_group, file = 'topics_output_', no_topics = 10):
    df = df.reset_index()
    
    f = open(file + file_group + '.txt', 'w')
    ...
    no_top_words = 10
    display_topics(lda, tf_feature_names, no_top_words, f)
    ...
    for index in range(len(topic_distribution.columns)):
        word_cloud_sklearn(index, lda, tf_feature_names, file_group)  
...
def classify_newdf(data_directory):
    df = pd.read_excel(data_directory + '2021-12-14_tweets_manual_automatic_nans.xlsx')
    print('Got data.')
    
    pipe = load(open('pipe.pkl', 'rb'))
    print('Loaded pipe.')
    
    df = preprocess(df, target_group = 'all', lemmatize = False, stem = True)
    
    # Predict values and save to df
    texts = df['processed text']
